...
import { UserPlus, Calendar, HeartPulse, CheckCircle2, ShieldAlert } from "lucide-react";
    ...
    <div className="flex bg-slate-50 min-h-screen">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[40%] bg-primary p-12 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-start gap-4 h-full">
          <Link href="/" className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 transition-colors rounded-full font-semibold backdrop-blur-md mb-8">
            <HeartPulse className="h-5 w-5 text-red-200" />
            VitalFlow
          </Link>
          
          <div className="mt-auto mb-16 max-w-md">
            <h1 className="text-4xl font-black mb-6 leading-tight">Your blood can save <br/><span className="text-red-200">up to three lives.</span></h1>
            <p className="text-white/80 text-lg mb-8 leading-relaxed">
              Every 2 seconds, someone in the country needs blood. By registering as a donor, you are taking the first step to becoming a real-life hero.
            </p>
            
            <ul className="space-y-4">
              <li className="flex items-center gap-3">
                <CheckCircle2 className="h-6 w-6 text-red-300 shrink-0" />
                <span>Receive notifications only for emergencies perfectly matching your profile.</span>
              </li>
              <li className="flex items-center gap-3">
                <CheckCircle2 className="h-6 w-6 text-red-300 shrink-0" />
                <span>Track your donation history and health impact seamlessly.</span>
              </li>
              <li className="flex items-center gap-3">
                <CheckCircle2 className="h-6 w-6 text-red-300 shrink-0" />
          ...
          <div className="text-sm text-red-200/60 mt-auto">
            &copy; {new Date().getFullYear()} VitalFlow Secure Registration Network
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-lg bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-slate-100 relative">
          ...
          {isRegistered ? (
             <div className="text-center py-8">
               ...
               <h2 className="text-3xl font-black mb-4 text-slate-900">Registration Complete</h2>
               <p className="text-slate-600 mb-8 max-w-sm mx-auto">
                 Thank you for stepping up to save lives. You are now officially a registered donor on VitalFlow.
               </p>
               <Link href="/dashboard/user" className="inline-block w-full py-3.5 bg-primary text-white font-bold rounded-xl shadow-lg hover:-translate-y-0.5 transition-transform text-center mb-4">
                 Go to My Dashboard
               </Link>
               <Link href="/donors" className="inline-block w-full text-center text-slate-500 font-medium hover:text-slate-900 py-2 transition-colors fade-in">
             ...
             <>
               <div className="mb-10 text-center lg:text-left">
                 ...
                 <p className="text-slate-500 font-medium">Please provide accurate details to help us match you during emergencies.</p>
               </div>
               ...
               <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                   <div>
                     ...
                     {errors.bloodGroup && <p className="text-red-500 text-sm mt-1.5 font-medium">{errors.bloodGroup.message}</p>}
                   </div>
                 ...
                 <div>
                   <label className="block text-sm font-semibold text-slate-700 mb-1.5">City / Area</label>
                   <input 
                     {...register("location")}
                   ...
                   />
                   {errors.location && <p className="text-red-500 text-sm mt-1.5 font-medium">{errors.location.message}</p>}
                 </div>

                 <div>
                   <label className="block text-sm font-semibold text-slate-700 mb-1.5">Last Donation Date (Optional)</label>
                 ...
                 <div className="pt-2">
                   <label className="flex items-start gap-4 cursor-pointer group bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-primary/30 transition-colors">
                     ...
                     <span className="text-sm font-medium text-slate-600 leading-snug">
                       I confirm that I am over 18, weigh at least 50kg, and agree to the <Link href="/terms" className="text-primary hover:underline font-bold">Terms of Service</Link> and <Link href="/privacy" className="text-primary hover:underline font-bold">Privacy Policy</Link>.
                     </span>
                   </label>
                   {errors.agreedToTerms && <p className="text-red-500 text-sm mt-2 ml-1 font-medium">{errors.agreedToTerms.message}</p>}
                 </div>
                 ...
                 <p className="text-center text-sm font-medium text-slate-500 mt-6">
                   Already registered? <Link href="/login" className="text-primary font-bold hover:underline">Log in</Link>
                 </p>
               </form>
             </>
          )}
